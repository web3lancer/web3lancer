...
import { databases, getUserProfile, getProfilePictureUrl, ID, storage } from '@/utils/api'; 
...
export default function DashboardPage() {
  ...
  const fetchLances = async (refresh = false) => {
    try {
      ...
      // In a real app, you would implement pagination with proper queries
      // For now, generating mock data
      setTimeout(() => {
        const mockLances: Lance[] = Array.from({ length: 10 }, (_, i) => ({
          $id: `lance-${page}-${i}`,
          userId: Math.random() > 0.3 ? (user?.$id || 'anonymous') : `user-${i}`,
          content: `This is a sample lance post #${page}-${i} about ${Math.random() > 0.5 ? 'Web3' : 'Blockchain'} development. ${Math.random() > 0.7 ? 'Check out this cool project I\'m working on!' : 'Looking for collaborators on a new DeFi protocol.'}`,
          likes: Math.floor(Math.random() * 100),
          comments: Math.floor(Math.random() * 20),
          shares: Math.floor(Math.random() * 10),
          createdAt: new Date(Date.now() - Math.floor(Math.random() * 7 * 24 * 60 * 60 * 1000)).toISOString(),
          visibility: Math.random() > 0.8 ? 'followers' : 'public',
          tags: Math.random() > 0.5 ? ['Web3', 'Blockchain', 'DeFi'].filter(() => Math.random() > 0.5) : undefined,
          userName: Math.random() > 0.3 ? (user?.name || 'Anonymous User') : `Web3 Developer ${i}`,
          userHandle: Math.random() > 0.3 ? (user?.$id || 'anon') : `web3dev${i}`,
          userProfilePic: Math.random() > 0.3 ? userProfilePic || undefined : undefined,
          ...
          media: Math.random() > 0.6 ? [{
            id: `media-${i}`,
            type: 'image',
            url: `https://picsum.photos/seed/${page}${i}/800/450`,
          }] : undefined,
        }));
        
        if (refresh) {
          setLances(mockLances);
        } else {
          setLances(prev => [...prev, ...mockLances]);
        }
        
        setIsLoading(false);
        // For demo purposes, limit pages to 5
        setHasMore(page < 5);
        if (!refresh) {
          setPage(prev => prev + 1);
        }
      ...
      // In a real implementation, you would use Appwrite's SDK like this:
      // const response = await databases.listDocuments(
      //   APPWRITE_CONFIG.DATABASES.POSTS || '', // Use your database ID
      //   APPWRITE_CONFIG.COLLECTIONS.POSTS || '', // Use your collection ID
      //   [
      ...
      //   ]
      // );
      // const fetchedLances = response.documents as unknown as Lance[];
      // setLances(prev => refresh ? fetchedLances : [...prev, ...fetchedLances]);
  ...
  const handlePostSubmit = async () => {
    ...
    try {
      // In a real app, you would upload media and create a post
      // For example:
      // const mediaIds = [];
      // for (const file of selectedMedia) {
      //   const response = await storage.createFile(
      //     APPWRITE_CONFIG.STORAGE.POST_MEDIA || '',
      //     ID.unique(),
      //     file
      //   );
      //   mediaIds.push({ id: response.$id, type: file.type.includes('image') ? 'image' : 'video', url: '' });
      // }
      // 
      // const newLance = await databases.createDocument(
      //   APPWRITE_CONFIG.DATABASES.POSTS || '',
      ...
      const mediaItems = selectedMedia.length > 0 
