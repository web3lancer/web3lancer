...
const WithdrawalForm: React.FC<WithdrawalFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    } else if (selectedWalletObj && amountNum > selectedWalletObj.balance) {
      newErrors.amount = `Insufficient balance. Available: ${formatCurrency(selectedWalletObj.balance, selectedWalletObj.currency)}`;
    }
  ...
  // Get selected wallet currency and balance
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <>
              ...
              {wallets.map((wallet) => (
                <option key={wallet.$id} value={wallet.$id}>
                  ...
                  {wallet.isDefault ? ' - Default' : ''} - Balance: {formatCurrency(wallet.balance, wallet.currency)}
                </option>
              ))}
            </>
          )}
      ...
      <div>
        <label htmlFor="amount" className="block text-sm font-medium text-gray-700">
          Amount {wallet && `(Max: ${formatCurrency(wallet.balance, wallet.currency)})`}
        </label>
